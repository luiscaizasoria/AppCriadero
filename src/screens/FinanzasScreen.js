    ...
    const cargar = async () => {
        ...
        try {
            ...
            const [
                ...
                obtenerMovimientosFinancieros(),
                obtenerConsolidadoCategorias(periodo),
                obtenerConsolidadoMensual(12)
            ]);
    ...
    return (
        ...
        >
            ...
            <View style={styles.consolidatedCard}>
                <Text style={styles.consolidatedHeading}>
                    Ingresos
                </Text>
                {
                    ingresosCategorias.length === 0
                        ? <Text style={styles.emptyLine}>Sin ingresos en el período.</Text>
                        : ingresosCategorias.map(item => (
                            <ConsolidatedRow
                                key={`I-${item.categoria}`}
                                label={nombreCategoria(item.categoria)}
                                value={item.total}
                            />
                        ))
                }

                <Text style={[styles.consolidatedHeading, styles.egressHeading]}>
                    Egresos
                </Text>
                {
                    egresosCategorias.length === 0
                        ? <Text style={styles.emptyLine}>Sin egresos en el período.</Text>
                        : egresosCategorias.map(item => (
                            <ConsolidatedRow
                                key={`E-${item.categoria}`}
                                label={nombreCategoria(item.categoria)}
                                value={item.total}
                            />
                        ))
            ...
            <View style={styles.consolidatedCard}>
                {
                    mensual.length === 0
                        ? <Text style={styles.emptyLine}>Sin datos mensuales.</Text>
                        : mensual.map(item => {
                            const balance =
            ...
            <View style={styles.tabs}>
                {
                    [
                        {id:'TODOS', text:'Todos'},
                        {id:'INGRESO', text:'Ingresos'},
                        {id:'EGRESO', text:'Egresos'}
                    ].map(item => (
                        <TouchableOpacity
                            key={item.id}
                            style={[
...

            {
                lista.map(item => (
                    ...
                    >
                        ...
                        <View style={styles.movementBottom}>
                            <Text
                                style={
                                    item.tipo === 'INGRESO'
                            ...
                            >
                                {item.tipo === 'INGRESO' ? '+' : '-'}${Number(item.valor).toFixed(2)}
                            </Text>
...
function nombreCategoria(categoria){
    const map = {
        VENTA_AVE:'Venta de aves',
        ENVIO_AVE:'Envíos de aves',
        VENTA_HUEVOS:'Venta de huevos',
        ALIMENTO:'Alimento',
        MEDICAMENTO:'Medicamentos',
        INFRAESTRUCTURA:'Infraestructura',
        COMPRA_AVE:'Compra de aves',
        OTRO:'Otros'
    };
    return map[categoria] || categoria || 'Otro';
}
